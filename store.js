...
const schema = {
  ...
  settings: {
    ...
    default: { commitmentMode: false, cooldownMinutes: 3, hideBalances: false }
  }
};
...
export const helpers = {
  ...
  // Week key YYYY-WW using ISO week starting Monday
  weekKey(date) {
...
export function addCasino(name) {
  const casinos = store.get('casinos');
  const id = helpers.id();
  casinos.push({ id, name: name.trim() });
  store.set('casinos', casinos);
  return id;
}

export function removeCasino(id) {
  store.set('casinos', store.get('casinos').filter(c => c.id !== id));
  store.set('entries', store.get('entries').filter(e => e.casinoId !== id));
}

export function addEntry({ dateISO, casinoId, spent, won, notes }) {
  const entries = store.get('entries');
  const id = helpers.id();
  entries.push({ id, dateISO, casinoId, spent: Number(spent) || 0, won: Number(won) || 0, notes: (notes || '').trim() });
...
export function setBudgets({ weekly, monthly }) {
  store.set('budgets', { weekly: Number(weekly) || 0, monthly: Number(monthly) || 0 });
}
...
export function clearAllData() {
  ...
  store.set('settings', { commitmentMode: false, cooldownMinutes: 3, hideBalances: false });
}
